ft_cfa <- function(data_files, model, estimator = "MLR", load_simulated, ordinal = FALSE) {
  ...
  arb_values <- numeric(length(data_files))  # ARB por réplica
  ...
  for (i in seq_along(data_files)) {
    ...
    # Validar que los datos se hayan leído correctamente
    if (is.null(data_sim)) next
    
    data_sim <- data_sim[, -1]  # Eliminar la primera columna porque contiene un listado de vbs
    colnames(data_sim) <- paste0("x", 1:ncol(data_sim))  # Renombrar columnas
